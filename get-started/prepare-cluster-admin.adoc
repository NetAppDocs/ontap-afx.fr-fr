...
NOTE: À partir d'ONTAP 9.19.1, les administrateurs SVM peuvent surveiller et gérer leurs propres SVM à l'aide d'un tableau de bord System Manager dédié. En savoir plus sur https://docs.netapp.com/us-en/ontap/concept-svm-admin-dashboard-sm.html["Tableau de bord administrateur SVM du gestionnaire de système ONTAP"^]
...
AFX impose plusieurs limitations, notamment des valeurs minimales et maximales, lors de la configuration et de l'utilisation de votre cluster. Ces limites se répartissent en plusieurs catégories.


CAUTION: Avant d’augmenter un paramètre opérationnel de votre système de stockage AFX, vous devez évaluer comment cela pourrait impacter vos applications ainsi que le cluster dans son ensemble. Par exemple, bien qu’augmenter le nombre de nœuds de contrôleur accroisse la capacité globale, cela pourrait également augmenter le trafic dans le réseau du cluster. Il est important de comprendre les implications de ces modifications de configuration et comment elles pourraient impacter votre environnement.

Nœuds de contrôleur par cluster:: Chaque cluster AFX doit comporter au moins quatre nœuds. Le nombre maximal de nœuds varie selon la version d'ONTAP ; par exemple, la version 9.19.1 RC autorise jusqu'à 32 nœuds par cluster AFX.
...
Commutateurs de cluster:: Vous avez besoin d'au moins deux commutateurs dans votre réseau de stockage en cluster.  La valeur maximale autorisée est déterminée en fonction du nombre total de nœuds de contrôleur dans le cluster.


Vous devriez consulter les informations disponibles dans le NetApp Hardware Universe et l’outil Interoperability Matrix pour déterminer les capacités de votre cluster AFX. Consultez également link:../release-notes/changes-to-limits-defaults.html["Modifications apportées aux limites et aux valeurs par défaut d'ONTAP"] dans la section des notes de version d’AFX.
